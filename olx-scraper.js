...
axios(url2)
    .then(response => {
        ...
        gridContainer.each(function () {
            ...
            const price = $(this).find('a > div > div > div> div.css-u2ayx9 > p').text().trim();
            const condition = $(this).find('a > div > div > div > div.css-iqvdlb > span > span > span').text();
            const location = $(this).find('a > div > div > div > div.css-odp1qd > p').text();
            // const date = $(this).find('.stats-table__main-stat').text();
